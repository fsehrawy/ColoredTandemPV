...
color=np.array([ref_solar.values[:,0],np.ones(2002)]) #wl=color[0,:], values=color[1,:]
...
for idx in np.arange(0, 2002):
    if (color[0,idx]>=color_step_wl_1) and (color[0,idx]<=color_step_wl_2):
        color[1,idx]=1-color_step_peak
...
solar_per_nm_colored = solar_per_nm*color[1,:]
solar_per_E_colored = solar_per_E*color[1,:]
...
AM15_colored = np.interp(Es, E[::-1], solar_per_E_colored[::-1])  # W m^-2 eV^-1
AM15flux_colored = AM15_colored / (Es*eV)  # number of photon m^-2 eV^-1 s^-1
...
class SQlim(object):
    def __init__(self, T=300, EQE_EL=1.0, intensity=1.0):
        ...
        self.AM15nm_colored = np.interp(self.WLs, WL, solar_per_nm_colored)
...
#        fluxcumm = cumtrapz(AM15flux[::-1], self.Es[::-1], initial=0)
    ...
    def available_E(self, Egs, E_MPP=True, xmin=300, xmax=2500,
        ...
        AM15nm_colored = np.interp(WLs, WL, solar_per_nm_colored)
        plt.figure(figsize=(8, 4.5))
        ax = plt.gca()
        # color options: darkgoldenrod, darkorange, yellow, black
        ...
        if numEg != 1:
            for n, Eg in enumerate(Egs[:-1]):
                ...
                SQ_E = SQlim(intensity=(Jscs[n]-Jscs[n+1]) / Jscs[n],T=self.T, EQE_EL=self.EQE_EL)
                para = SQ_E.get_paras(Eg, toPrint=False)
                factor = para["Voc"] * para["FF"] / 100.0 / Eg           
                mask = ((1240.0/WLs) >= Eg) * (Egs[n+1] >= (1240.0/WLs))
                Eavail = factor * AM15nm_colored / (1240.0/WLs) * Eg * mask
...
#    SQ.plotall()
    # SQ.E_loss(1.125)
    # SQ.E_loss(1.9)
    # SQ.available_E(1.125) # Eg of PSC is 1.5 - 2.2
#
...
#   Triple-junction solar cells GaInP/InGaAs/Ge
    SQ.available_E(Egs=[0.65,1.4,1.9])
